...
/mob/living/scp1366/proc/induce_calm_aura()
	for(var/mob/living/L in view(calm_radius, src))
		...
		if(!(L in affected_mobs))
			L.visible_message(SPAN_NOTICE("[L] suddenly feels a wave of calm wash over them."))
			affected_mobs += L

/mob/living/scp1366/attack_hand(mob/living/carbon/human/H)
	if(H.a_intent == I_HELP)
		to_chat(H, SPAN_NOTICE("As you touch Uncle Styopa, your wounds begin to heal."))
...
/mob/living/scp1366/verb/tell_story()
	...
	if(usr.a_intent != I_HELP)
		to_chat(usr, SPAN_NOTICE("Uncle Styopa only shares stories with those who show a willingness to listen."))
		return
	...
	to_chat(usr, SPAN_NOTICE("Uncle Styopa begins to tell you a story: [story]"))
