...
/obj/scp263/proc/question_fail(is_timeout = FALSE)
	...
	spawn(10 SECONDS)
		state = STATE_IDLE
		update_icon()
		current_scp263_1.say("[pick("Очень жаль, мне это очень понравилось.", "Их неудача - настоящая жалость.")] Посмотрим, удастся ли следующему участнику избежать провала... и сорвать куш!")
		contestant.dust()
