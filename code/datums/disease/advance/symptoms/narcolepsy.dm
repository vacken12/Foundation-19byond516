...
/datum/symptom/scp714/Start(datum/disease/advance/A)
	if(!..())
		return
	if(A.properties["stealth"] >= 1)
		suppress_warning = TRUE
	if(A.properties["transmittable"] >= 1) //yawning (mostly just some copy+pasted code from sneezing, with a few tweaks)
		yawning = TRUE
	if(A.properties["stage_speed"] >= 10) //act more often
		symptom_delay_min = 10
		symptom_delay_max = 20

/datum/symptom/scp714/Activate(datum/disease/advance/A)
	if(!..())
		return
	var/mob/living/M = A.affected_mob
	switch(A.stage)
...
/datum/symptom/scp714/End(datum/disease/advance/A)
	A.affected_mob.adjust_drowsiness(-30 SECONDS)
	return ..()
